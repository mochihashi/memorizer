...
function readFile(file) {
	...
	reader.addEventListener('load', function() { parseFile(new Uint8Array(reader.result), file.name); });
}
function parseFile(unit8arr, name) {
...
function parseText(text, name) {
	if(name) {
		...
		document.title = shortName + " - Memorizer";
		if(args['data']) shortName = '<a href="' + args['data'] + '">' + shortName + '</a>';
		el('filename').innerHTML = shortName;
	}
	...
	if(name) {
		arr = [];
		arr.push(data.title.join("\t"));
		for(var r = 0; r < data.list.length; r++) {
			var row = [];
			for(var c = 0; c < data.list[r].length; c++) {
				var val = data.list[r][c];
				if(val) {
		...
		el('dictionary-table').innerHTML = arr.join("\n");
	}
...
function onload() {
	...
	if(args['file']) {
		...
		http.onload = function(e) { if(http.status == 200) parseFile(new Uint8Array(http.response), args['file']); };
		http.send();
	}
	if(args['data']) {
		var http = new XMLHttpRequest(); http.open('GET', args['data'], true);
		var idx = args['idx'];
		http.onload = function(e) {
			if(http.status != 200) return;
			var html = http.responseText;
			var tag = '<div class="tsv-data"' + (idx ? (' idx="' + idx + '"') : '') + '>';
			var p = html.indexOf(tag);
			if(p < 0) return;
			p += tag.length;
			var p2 = html.indexOf('</div><!-- .tsv-data -->', p);
			var text = html.slice(p, p2).trim();
			parseText(text, args['data']);
		};
		http.send();
	}
